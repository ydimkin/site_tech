...
const STATS = [
  ...
  { value: '20+', label: 'Курсов', icon: BookOpen },
  { value: '15', label: 'Педагогов', icon: Award },
...
export default function HomePage() {
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center bg-hero-gradient overflow-hidden">
        ...
        <div className="container-page relative z-10 py-24">
          <div className="max-w-3xl">
            <div className="badge-primary badge mb-6 animate-fade-in">
              🚀 Набор открыт — запишитесь сейчас!
            </div>

            <h1 className="text-5xl md:text-7xl font-bold text-white font-display leading-tight animate-slide-up mb-6">
              Технологии{' '}
              <span className="gradient-text">будущего</span>
              {' '}для ваших детей
            </h1>

            <p className="text-xl text-slate-400 leading-relaxed max-w-2xl animate-slide-up mb-10" style={{ animationDelay: '0.1s' }}>
            ...
            {/* Stats */}
            <div className="grid grid-cols-4 gap-4 mt-16 animate-fade-in" style={{ animationDelay: '0.4s' }}>
              {STATS.map(({ value, label, icon: Icon }) => (
                <div key={label} className="text-center">
                  <div className="text-2xl md:text-3xl font-bold text-white font-display">{value}</div>
      ...
      {/* ===== DIRECTIONS ===== */}
      <section className="py-24 bg-[#080d1b]">
        ...
        <div className="container-page">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              ...
              <div key={title} className="card p-6 text-center hover:border-primary-500/30 transition-colors">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
              {teachers.map((t) => (
                <div key={t.id} className="card-hover p-6 text-center group">
                  <div className="w-20 h-20 rounded-2xl bg-primary-gradient mx-auto mb-4 flex items-center justify-center text-white text-2xl font-bold group-hover:scale-105 transition-transform">
                    {t.photo_url ? (
                  ...
                  <p className="text-slate-400 text-xs mt-1">{t.position}</p>
                  {t.experience > 0 && (
              ...
              {news.map((item) => (
                <Link key={item.id} to={`/news/${item.id}`} className="card-hover group overflow-hidden">
                  ...
                  <div className="p-5">
                    ...
                    {item.preview && <p className="text-slate-400 text-xs mt-2 line-clamp-2">{item.preview}</p>}
                  </div>
                </Link>
              ))}
            </div>
